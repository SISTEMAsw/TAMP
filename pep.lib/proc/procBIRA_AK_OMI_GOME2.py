...
def createImgBIRA_AK_OMI_GOME2(fileAbsPath ):
    ...
    for i in range(0,len(lat),stepSize):
        ...
        if instrument == 'GOME2B':
            ...
            command_call = [workingDir + 'bin/remap', '-i', tmpOutFile_2D, '-l', str(upper_left[0]), str(upper_left[1]), '-e', str(lower_right[0])+','+ str(lower_right[1]), '-a', filenameCoords, '-s', str(pixelSize),'-w',str(window), '-n', str(no_data), '-q', '-o', filenameOutput_2D+ '_mask','-f','50000']
...
if __name__ == '__main__':
    ...
    createImgBIRA_AK_OMI_GOME2(fileAbsPath) 

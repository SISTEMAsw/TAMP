...
def extractProduct(fileAbsPath, grbs_name, grbs_typeOfLevel):
    ...
    prefix = 'AROME_'+a_data[0]['name'].replace(' ','_')+'_'+a_data[0]['typeOfLevel']+'_'
    dateString = str(a_data[0]['dataDate'])+str(a_data[0]['dataTime']).zfill(4)

    for i in range(numBands):
        #add single band in multibads dataset
        ...
        try:
            ...
            outputFileName = prefix+dateString+'_'+val[0]+'.tif'
...
def getCoords(fileAbsPath):
    #non e' la procedura corretta ma funziona
    grbs = pygrib.open(fileAbsPath)
    temperature_start = 64
    a_temperature = grbs[temperature_start]
    grbs.close()
    #RETURN LOWER LEFT UPPER RIGHT!! THE COORDS IN THIS TUPLE IS FIPPED ON UP DOWN. THE 'latitudeOfFirstGridPointInDegrees' INDEED IS THE  LAST POINT!!!!
...
def getDateTime(fileAbsPath):
    ...
    startDateTime = (datetime.strptime(dateTime, "%Y%m%d%H%M") + timedelta(hours = int(val[0]))  ).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def createImgAROME(fileAbsPath):
    outFileList = {}

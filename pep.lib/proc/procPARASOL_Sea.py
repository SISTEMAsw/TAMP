...
def read_records_oc(datafile):
    ...
    for i in range(2, recordsNumber+2):
        ...
        #to handle corrupted input data
        if rec != oldRec + 1:
            rec = oldRec + 1
            fin.seek(pos)
            fin.read(3)
        ...
        fin.seek(pos+20)
        aot_865 = struct.unpack('>H', fin.read(2))[0]
        aot_670 = struct.unpack('>H', fin.read(2))[0]
        
        lines.append(line)
        cols.append(col)
        aot_670s.append(aot_670)
        aot_865s.append(aot_865)

        pos += recordsLength        
        oldRec = rec        
    return lines, cols, aot_865s, aot_670s


def createImgPARASOL_Sea(fileAbsPath, pixelSize = 0.15):
    ...
    #filename = 'PARASOL_' + os.path.basename(fileAbsPath)[2:4] + '_AOT_SEA_' + timeStart[0:8] + '.' + timeStart[8:-2] + '.tif'
    filename = 'PARASOL_' + os.path.basename(fileAbsPath)[2:4] + '_AOT_SEA_' + timeStart[0:8] + '.' + timeStart[8:-2] + '_tmp.tif'
    #filenameOutput = filename[:-4] + '_REMAPPED.tif'
    filenameOutput = filename[:-8] + '.tif'
    
    lines, cols, aot_670, aot_865 = read_records_oc(fileAbsPath)
    ...
    print filenameCoords, rangeY, rangeX, 2, gdal.GDT_Float32
    print type(filenameCoords),type(rangeY),type(rangeX),type(2),type(gdal.GDT_Float32) 
...
if __name__ == '__main__':    
    ...
    for file in files:
        ...
        createImgPARASOL_Sea(os.path.abspath(file))

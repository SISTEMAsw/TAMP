...
def extractEEAData(fileAbsPath):    
    ...
    outFileHourly.write('<EEA>\n')
    outFileHourly.write('    <siteName>' + station["station"] + '</siteName>\n')
    outFileHourly.write('    <siteLongitude>' + str(station["longitude"]) + '</siteLongitude>\n')
    outFileHourly.write('    <siteLatitude>' + str(station["latitude"]) + '</siteLatitude>\n')
    outFileHourly.write('    <siteElevation>' + str(station["height"]) + '</siteElevation>\n')
    ...
    outFileAvg.write('<EEA>\n')
    outFileAvg.write('    <siteName>' + station["station"] + '</siteName>\n')
    outFileAvg.write('    <siteLongitude>' + str(station["longitude"])  + '</siteLongitude>\n')
    outFileAvg.write('    <siteLatitude>' + str(station["latitude"]) + '</siteLatitude>\n')
    outFileAvg.write('    <siteElevation>' + str(station["height"]) + '</siteElevation>\n')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        sys.exit('\nUsage: %s EEA' % sys.argv[0])
    else:
        if not os.path.exists(sys.argv[1]):
            sys.exit('\nERROR: File %s was not found!\n' % sys.argv[1])
            
    extractEEAData(sys.argv[1])
